...
if __name__ == "__main__":
    ...
    ax.semilogy(lossSimAA, '-->',label='SimGDA-RAM, m=10',markevery=markevery)
    ax.semilogy(lossSimAA1, '-->',label='SimGDA-RAM, m=20',markevery=markevery)
    ax.semilogy(lossSimAA2, '-->',label='SimGDA-RAM, m=40',markevery=markevery)
    ax.semilogy(lossaltAA, '-d',label='AltGDA-RAM, m=10',markevery=markevery)
    ax.semilogy(lossaltAA1, '-d',label='AltGDA-RAM, m=20',markevery=markevery)
    ax.semilogy(lossaltAA2, '-d',label='AltGDA-RAM, m=40',markevery=markevery)
    
    # ax.set_ylim([1e-15,1e4])
    ax.legend(loc=0, fontsize=15)
    ax.set_xlabel('Iteration',fontsize=12)
    ax.set_ylabel('Loss',fontsize=12)
    fig.savefig('bipsupp1.png', dpi=300, bbox_inches = 'tight', pad_inches = 0)
    
    fig, ax = plt.subplots(figsize=(7,7))
    klist = [5,10,50,100,200,500]
    for k in klist:
        lossSimAA = simAA(problem, x0, y0, k, lr, maxiter, True)
        ax.semilogy(lossSimAA, '-->',label='SimGDA-RAM, m='+str(k),markevery=markevery)# ax.set_ylim([1e-15,1e4])
    ax.legend(loc=5, fontsize=15)
    ax.set_ylim([1e-28,1e4])
    ax.set_xlabel('Iteration',fontsize=12)
    ax.set_ylabel('Loss',fontsize=12)
    fig.savefig('bipsupp2.png', dpi=300, bbox_inches = 'tight', pad_inches = 0)
    ...
    for k in klist:
        lossaltAA = altAA(problem, x0, y0, k, lr, maxiter, True)
        ax.semilogy(lossaltAA, '-d',label='AltGDA-RAM, m='+str(k),markevery=markevery)# ax.set_ylim([1e-15,1e4])
    ax.legend(loc=0, fontsize=15)
    ax.set_ylim([1e-28,1e4])
    ax.set_xlabel('Iteration',fontsize=12)
    ax.set_ylabel('Loss',fontsize=12)
    fig.savefig('bipsupp3.png', dpi=300, bbox_inches = 'tight', pad_inches = 0)